...
function renderShop() {
    ...
    SHOP_ITEMS.forEach(item => {
        ...
        } else if (item.id === "item_red_card") {
            countDisplay = `<div style="font-size:12px; color:#e74c3c; font-weight:bold;">持有：${redCardCount}</div>`;
        } else if (item.id === "item_green_card") {
            countDisplay = `<div style="font-size:12px; color:#27ae60; font-weight:bold;">持有：${greenCardCount}</div>`;
        }
        ...
        if (item.type === 'buff') {
            ...
            // ========== 新增：紅綠卡的特殊按鈕狀態 (冷卻與持有上限) ==========
            if (item.id === "item_red_card") {
                if (redCardCount >= 1) {
                    canBuy = false;
                    btnText = `已達上限 (1/1)`;
                } else if (redCardCooldown > 0) {
                    canBuy = false;
                    btnText = `冷卻中 (${redCardCooldown}回合)`;
                    btnClass += " disabled-cooldown";
                }
            } else if (item.id === "item_green_card") {
                if (greenCardCount >= 1) {
                    canBuy = false;
                    ...
                    canBuy = false;
                    btnText = `冷卻中 (${greenCardCooldown}回合)`;
                    btnClass += " disabled-cooldown";
                }
            }
            // =================================================================

            // 負債狀態檢查 (優先級最高，覆蓋上面的文字)
            if (loan > 0) {
                canBuy = false;
                btnText = `需先還清負債`;
            }
