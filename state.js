...
let pendingTradeImpacts = STOCKS.map(() => 0); // deferred impacts applied next turn
let aiVisibleEvents = []; // snapshot of last turn's published news — what AI may act on
...
let aiPlayers = AI_PLAYER_TEMPLATES.map(t => ({
    ...
    bank: 0,            // savings deposit earning interest each turn
    fundUnits: 0,       // index fund units held
    fundTotalCost: 0,   // cost basis for fund P&L tracking
}));
...
function getAiTotalAssets(ai) {
    return ai.cash
        + ai.bank
        + ai.holdings.reduce((sum, qty, i) => sum + qty * getCurrentPrice(i), 0)
        + ai.fundUnits * fundNAV;
}